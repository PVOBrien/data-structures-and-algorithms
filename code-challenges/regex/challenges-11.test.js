...
const toLastNames = people => {
  let newPeople = people.map(eachPerson => {
    return `${eachPerson.firstName} ${eachPerson.lastName}`;
  });
  return newPeople;
};
...
const validateEmail = (email) => {
  let regex = /^\w+.\w+@\w+[\W]{1}([com]+$|[org]+$|[net]+$)/gm;
...
const validatePhoneNumber = (phoneNumber) => {
  let regex = /^(\({1}\d{3}\){1}|\d{3})(\s?|-?)\d{3}(\s?|-?)\d{4}$/gm;
