...
const createServer = () => {
  ...
  app.get('/aboutme', (request, response) => {
    response.status(200).send('Here I am!');
  });

  app.get('/favoritefoods', favoriteFoodHandlers);

  var server = app.listen(3301, function () {
    var port = server.address().port;
    // console.log('Example app listening at port', port); //working through other challenges, don't want a spare console function running atm
  });
  return server;
};

function helloHandler (request, response) {
  response.status(200).send('howdy, howdy.');
}

function favoriteFoodHandlers (request, response) {
  response.status(200).send('Love me some breakfast cereal.');
}
...
const count = (target, input) => {
  ...
  input.forEach(array => {
    newArray.push(array.filter(item => item === target));
  });

  newArray.forEach(item => {
    item.forEach(element => element === target ? targetCount++ : false);
  });
  return targetCount;
};
