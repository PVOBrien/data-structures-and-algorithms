...
const createServer = () => {
  ...
  app.get('/', (request, response) => {
    response.status(200).send('good morning!');
  });

  app.delete('/things/1', deleteIt);

  function deleteIt(request, response) {
    response.status(405).send('we out');
  }
...
const toTitleCase = (arr) => {
  let capitalWord = arr.reduce((accumulator, word) => {
    return accumulator.concat(upperCaseFirst(word));
  }, []);
  return capitalWord;
}
