...
const firstLetters = (arr) => {
  let letterOne = []
  arr.forEach ( run => {
    letterOne.push(run.substring(0,1));
  })
  return letterOne;
};
...
const findHappiness = (arr) => {
  let smiley = [];
  arr.forEach ( run => {
    run.includes(`:)`) ? smiley.push(run) : null;
  })
  return smiley;
};
...
const standardizePhoneNumbers = (arr) => {
  let numericPhone = arr.map(each => {
    let segmentOne = each.substring(1,4);
    let segmentTwo = each.substring(6,9);
    let segmentThree = each.substring(10,14);
