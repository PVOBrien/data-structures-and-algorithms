...
const templatingWithMustache = () => {
  characters.forEach(one => {
    new Character(one).characterCreator();
  });
...
function Character(persona) {
  this.name = persona.name;
  this.spouse = persona.spouse;
  this.children = persona.children;
  this.house = persona.house;
}

Character.prototype.characterCreator = function() {
...
const getCourseKeys = (obj) => {
  let courseArray = [];
  for (let each in courseInfo) {
    courseArray.push(each);
  }
  return courseArray;
};
...
const hasChildrenValues = (arr, character) => {
  characters.forEach(persona => (Object.values(persona.children))); // in repl, this finds the children in each array property. Don't know how to get it to go truthy or falsy. Thought this was built in?
};
